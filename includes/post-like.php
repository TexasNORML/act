...
function like_scripts() {
	wp_register_script( 'post-like', get_template_directory_uri().'/js/custom/post-like.js', array('jquery'), '1.0', 1 );
	wp_localize_script( 'post-like', 'ajax_var', array(
...
function getPostLikeLink( $post_id ) {
	...
	if ( AlreadyLiked( $post_id ) ) {
		...
		$heart = '<i  class="fa fa-heart icon-like "></i>';
	} else {
		$class = esc_attr( '' );
		$title = esc_attr( 'Like' );
		$heart = '<i class="fa fa-heart-o icon-unlike "></i>';
	}
	$output = '<a href="#" class="jm-post-like entry-like'.$class.'" data-post_id="'.$post_id.'" title="'.$title.'">'.$heart.' <small>'.$count.'</small></a>';
	return $output;
}
...
function show_user_likes( $user ) { ?>        
    <table class="form-table">
        <tr>
			<th><label for="user_likes"><?php esc_html_e( 'You Like:', 'invento' ); ?></label></th>
			<td>
            <?php
			...
			if ( $count > 0 ) {
				...
				echo "<p>" . esc_html_e( 'You don\'t like anything yet.', 'invento' ) . "</p>\n";
			} ?>
            </td>
		</tr>
    </table>
...
function frontEndUserLikes() {
	if ( is_user_logged_in() ) { // user is logged in
		...
		if ( $count > 0 ) {
			...
			$like_list .= "<aside>\n";
			$like_list .= "<h3>" . esc_html__('You Like:', 'invento' ) . "</h3>\n";
			$like_list .= "<ul>\n";
...
function jm_most_popular_today() {
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . esc_html_e( 'Today\'s Most Popular Posts', 'invento' ) . "</h3>\n";
		echo "<ul>\n";
...
function jm_most_popular_month() {
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . esc_html_e( 'This Month\'s Most Popular Posts', 'invento' ) . "</h3>\n";
		echo "<ul>\n";
		while ( $pop_posts->have_posts() ) {
			$pop_posts->the_post();
			echo "<li><a href='" . get_permalink($post->ID) . "'>" . get_the_title() . "</a></li>\n";
...
function jm_most_popular_week() {
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . esc_html_e( 'This Week\'s Most Popular Posts', 'invento' ) . "</h3>\n";
		echo "<ul>\n";
		while ( $pop_posts->have_posts() ) {
			$pop_posts->the_post();
			echo "<li><a href='" . get_permalink($post->ID) . "'>" . get_the_title() . "</a></li>\n";
...
function jm_most_popular() {
	global $post;
	echo "<aside>\n";
	echo "<h3>" . esc_html_e( 'Most Popular Posts', 'invento' ) . "</h3>\n";
	echo "<ul>\n";
	$args = array(
		 'post_type' => array( 'post', 'enter-your-comma-separated-post-types-here' ),
		 'meta_query' => array(
